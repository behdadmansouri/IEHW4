...
def admin_movie_update_delete(role, movie_id):
    try:
        ...
        if request.method == 'PUT':
            ...
            if (name is not None and description is not None and
                    type(name) == str and type(description) == str):
                db_query('UPDATE MOVIE SET NAME="%s", DESCRIPTION="%s" WHERE ID=%i'
                         % (name, description, int(movie_id)))
            else:
                return make_response({'message': 'Bad Request'}, 400)
        elif request.method == 'DELETE':
            db_query('DELETE FROM MOVIE WHERE ID=%i' % int(movie_id))
    ...
    try:
        ...
        if request.method == 'PUT':
            ...
            if approved is not None and type(approved) == bool:
                db_query('UPDATE COMMENTS SET APPROVED={0} WHERE ID={1}'.format(approved, comment_id))
            else:
                return make_response({'message': 'Bad Request'}, 400)

        elif request.method == 'DELETE':
            db_query('DELETE FROM COMMENTS WHERE ID=%i' % int(comment_id))
...
def user_vote(role):
    try:
        ...
        if (movie_id is not None and vote is not None and
            ...
            db_query('INSERT INTO COMMENTS (USER_ID, MOVIE_ID, COMMENT) VALUES (%i, %i, %d)'
                     % (int(user_id), int(movie_id), float(vote) / 10))
            return make_response({'message': 'ok'}, 204)
        else:
            return make_response({'message': 'Bad Request'}, 400)
    # SUCCESS
